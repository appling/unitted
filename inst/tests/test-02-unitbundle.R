...
test_that("unitbundles can be created", {
  ...
  expect_that(new("unitbundle", unitstr="kg ha^-1 yr^-2"), equals(unitbundle(unitstr="kg ha^-1 yr^-2")))
  # Explicit parameter names
...
test_that("unitbundles can be inspected", {
  ...
  expect_that(isTRUE(is.na(get_units(list(unitbundle(),unitbundle())))), is_true())
